...
int main(int argc, char *argv[])
{
    ...
    managed_shared_memory o(open_only, "MySharedMemory");
    MyShmStringVector *lol = o.find<MyShmStringVector>("myshmvector").first;
    for(int i=0; i<lol->size(); i++)
        std::cout << (*lol)[i] << std::endl;

    MyShmString hue(charallocator);
    hue = "hue";
    myshmvector->push_back(hue);
